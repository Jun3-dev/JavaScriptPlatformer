...
*/
  ...
  [
   ...
   [
    ...
    "    T   T   T   T   T        G                    TT",
    "Z                       _____G                                     Q",        
    "G   G   G   G   G   G   G   G   G   G   G   G   G   G   G __    G G G",
...
Player.updateY = function() {
  ...
  if (this.ySpeed < 4 || keys[UP_ARROW] || keys[87]) {
    //to make higher jumps the longer the up arrow is pressed
    this.ySpeed -= gravity;
  } else {
...
Portal.draw = function() {
  if (!antilag) {
    if (
      Math.abs(this.x - Player.x - Player.w < 400) &&
      Math.abs(this.y - Player.y - Player.h) < 250
    ) {
      /*legacy code
  fill(22, 184, 224);
...
function win() {
  background(245);
  textSize(width / 16);
  text("You beat all the levels.", width / 2, height / 2);
  noLoop();
  return;
}
function lose() {
